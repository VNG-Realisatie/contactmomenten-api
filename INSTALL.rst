...
2. Get the code:
   ...
   .. code-block:: bash
       ...
       $ git clone git@github.com:VNG-Realisatie/contactmomenten-api contactmomenten
       $ cd contactmomenten
...
``src/contactmomenten/conf/local.py``. You can base this file on the
...
.. code-block:: bash
    ...
    $ python src/manage.py test contactmomenten
...
1. Clone or download the code from `Github`_ in a folder like
   ...
   .. code-block:: bash
       ...
       $ git clone git@bitbucket.org:VNG-Realisatie/contactmomenten-api contactmomenten
       Cloning into 'contactmomenten'...
       ...
...
2. Start the database and web services:
   ...
   .. code-block:: bash
       ...
       $ docker-compose up -d
       Starting contactmomenten_db_1 ... done
       Starting contactmomenten_web_1 ... done
   ...
   .. code-block:: bash
       ...
       $ docker logs -f contactmomenten_web_1
...
3. Create an admin user and load initial data. If different container names
   ...
   .. code-block:: bash
       ...
       $ docker exec -it contactmomenten_web_1 /app/src/manage.py createsuperuser
       Username: admin
       ...
       Superuser created successfully.

       $ docker exec -it contactmomenten_web_1 /app/src/manage.py loaddata admin_index groups
...
environment variables. See ``src/contactmomenten/conf/docker.py`` for
all settings.
...
.. code-block:: bash
    ...
    $ docker build . && docker run \
        -p 8000:8000 \
        -e DJANGO_SETTINGS_MODULE=contactmomenten.conf.docker \
        -e DATABASE_USERNAME=... \
        -e DATABASE_PASSWORD=... \
        -e DATABASE_HOST=... \
        --name contactmomenten

    $ docker exec -it contactmomenten /app/src/manage.py createsuperuser

Settings
========

All settings for the project can be found in
``src/contactmomenten/conf``.
